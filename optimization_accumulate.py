...
def create_optimizer(loss, init_lr, num_train_steps, num_warmup_steps, use_tpu, mixed_precision=False):
    ...
    accum_vars = [tf.Variable(tf.zeros_like(
        tv.initialized_value()), trainable=False) for tv in tvars]
    ...
    accum_ops = [accum_vars[i].assign_add(gv) for i, gv in enumerate(grads)]

    train_op = optimizer.apply_gradients(
        [(accum, tvar) for accum, tvar in zip(accum_vars, tvars)])
